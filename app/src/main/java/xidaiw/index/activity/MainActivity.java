...
import xidaiw.util.AndroidBug54971Workaround;
...
public class MainActivity extends SlidingFragmentActivity implements View.OnClickListener{
   ...
   //底部四个文字
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        AndroidBug54971Workaround.assistActivity(findViewById(android.R.id.content));//适配华为手机虚拟键遮挡底部选项栏的bug

        getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);//状态栏沉浸

        initView();//初始化views

        createFrames();//创建四个fragment实例

        initMenu(); //进入APP首先加载indexFragment页面
        //底部导航的点击事件
...
//切换到首页
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.tv_logout:
                ...
                editor.clear().commit();
                AsyncDialog.show(this, "正在退出...");
                HttpClient.get(this, Urls.getLogout(), new LogoutAsyncHttpResponseHandler());//登录操作
                break;
            case R.id.iv_avanty:
                menu.toggle();
    ...
    public class LogoutAsyncHttpResponseHandler extends AsyncHttpResponseHandler {
        public void onStart() {
            super.onStart();
        }
