...
public class LoansAdapter extends BaseAdapter{
	...
	public LoansAdapter(Context context, List<LoanPageAdapter.DataBean.List1Bean> loanList){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//holder.productName.setText(loanList.get(position).getProductName().split("第")[0]);
		...
		holder.totalAmount.setText(GlobalUtils.NumberFormatTransfer(loanList.get(position).getTotalAmount()+""));
...
//				if (loanList.get(position).getProductTypeName()=="常规固定期限产品"){
					...
					holder.finishRatio.setProgress(Float.parseFloat(df.format(loanList.get(position).getFinishRatio())));
